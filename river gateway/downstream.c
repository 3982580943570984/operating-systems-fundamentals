...
int main([[maybe_unused]] int argc,[[maybe_unused]] char ** argv)
{
  ...
  int gateway_downstream_fd = open(gateway_downstream_fifo, O_RDONLY); 
  ...
  while (true) {
    ...
    int terminal_count;
    read(gateway_downstream_fd, &terminal_count, sizeof(int));
    ...
    for (int i = 0; i < terminal_count; ++i) {
      read(gateway_downstream_fd, &terminal_indices[i], sizeof(int));
      printf("terminal_index = %d\n", terminal_indices[i]);
    }

    /* Сообщаем о желаемых к использованию терминалах */
    write(downstream_gateway_fd, &terminal_count, sizeof(int));
    for (int i = 0; i < terminal_count; ++i) {
      write(downstream_gateway_fd, &terminal_indices[i], sizeof(int));
    }
